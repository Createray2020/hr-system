...
function normalizeDate(row) {
  const raw = row['Start Date'] ?? row.date ?? row['西元日期'] ?? row.Date ?? null;
  if (raw == null) return null;
  const s = String(raw).trim();
  if (!s) return null;
  // Accept slash/dash separated with single or double digit M/D:
  //   2026/1/1、2026/01/01、2026-1-1、2026-01-01
  const m = s.match(/^(\d{4})[\/\-](\d{1,2})[\/\-](\d{1,2})$/);
  if (m) {
    return `${m[1]}-${m[2].padStart(2, '0')}-${m[3].padStart(2, '0')}`;
  }
  // Fallback: 8 digit pure number 20260101
...
function pickDescription(row) {
  // Google Calendar 'Subject' 欄是節日名（中文）
  return row.Subject ?? row.description ?? row['名稱'] ?? row.name ?? '';
}

function pickNote(row) {
  // Google Calendar 的 'Description' 是 note（跟 Subject 是兩個欄位）
  return row.Description ?? row['備註'] ?? row.note ?? row.holidayCategory ?? null;
}

function pickIsHoliday(row) {
  // Google Calendar 'All Day Event' = True 視同放假日（行事曆條目本來就是放假日）
  if (row['All Day Event'] !== undefined && row['All Day Event'] !== '') {
    const v = String(row['All Day Event']).trim().toLowerCase();
...
function mapHolidayType(row) {
  ...
  // 例假日（Google Calendar 把每個週六日標記）— 不是真的國定假日、跳過
  if (desc === '例假日' || /^例假日/.test(desc)) return null;
...
function isExplicitlyUnknown(row) {
  ...
  // 例假日 是已知 skip 類型、不該 warn
  if (desc === '例假日' || /^例假日/.test(desc)) return false;
  return true;
}
