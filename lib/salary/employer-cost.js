...
export function calculateEmployerCost({
  ...
  // labor: direct premium 優先、否則 insured × rate;最後 × ratio
  const rawLabor = laborCompanyPremium != null
    ? laborCompanyPremium
    : ((insuredSalaryLabor || 0) * (laborRate || 0));
  const employer_cost_labor = Math.round(rawLabor * ratio);

  // health: 同上
  const rawHealth = healthCompanyPremium != null
    ? healthCompanyPremium
    : ((insuredSalaryHealth || 0) * (healthRate || 0));
  ...
  // 三項以勞保投保為基準
  const employer_cost_occupational = Math.round((insuredSalaryLabor || 0) * (occupationalRate || 0) * ratio);
  const employer_cost_employment   = Math.round((insuredSalaryLabor || 0) * (employmentRate   || 0) * ratio);
