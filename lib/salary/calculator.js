...
export async function calculateMonthlySalary(repo, { employee_id, year, month, callerId = null }) {
  ...
  const deduct_labor_ins_auto = !hasInsurance
    ? 0
    : (laborEmpDirect != null
        ? Math.round(laborEmpDirect)
        : Math.round((insuredSalaryLabor || 0) * TW_2026_LABOR_INS_EMPLOYEE_RATE));
  const deduct_health_ins_auto = !hasInsurance
    ? 0
    : (healthEmpDirect != null
        ? Math.round(healthEmpDirect)
        : Math.round((insuredSalaryHealth || 0) * TW_2026_HEALTH_INS_EMPLOYEE_RATE));
