...
export async function calculateMonthlySalary(repo, { employee_id, year, month, callerId = null }) {
  ...
  const grade_allowance_pro   = round2((Number(emp.grade_allowance)   || 0) * proRataRatio);
  const manager_allowance_pro = round2((Number(emp.manager_allowance) || 0) * proRataRatio);
