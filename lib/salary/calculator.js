...
import { computeNightAllowance } from './night-allowance.js';
...
export async function calculateMonthlySalary(repo, { employee_id, year, month, callerId = null }) {
  ...
  const nightShifts = typeof repo.findNightShiftSchedulesByMonth === 'function'
    ...
    : [];
  const night_allowance_auto = computeNightAllowance(nightShifts);
  ...
  const rawLaborInsEmp = !hasInsurance ? 0
    : (laborEmpDirect != null
        ? laborEmpDirect
        : (insuredSalaryLabor || 0) * TW_2026_LABOR_INS_EMPLOYEE_RATE);
  ...
  const rawHealthInsEmp = !hasInsurance ? 0
    : (healthEmpDirect != null
        ? healthEmpDirect
        : (insuredSalaryHealth || 0) * TW_2026_HEALTH_INS_EMPLOYEE_RATE);
