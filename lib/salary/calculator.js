...
export async function calculateMonthlySalary(repo, { employee_id, year, month, callerId = null }) {
  ...
  const proRataRatio = isFinalMonth && totalDaysInMonth > 0
  ...
  // prorata_base:離職月寫 base × ratio、非離職月 null(GENERATED COALESCE 自動回 base)
  const prorata_base = isFinalMonth ? round2(base_salary * proRataRatio) : null;
  ...
  const grossPreTax =
    (isFinalMonth ? Number(prorata_base) : base_salary)
    + Number(ab.actual || 0)
    + grade_allowance_pro
