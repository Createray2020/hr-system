...
/**
 * 2026 年(中華民國 115 年)— 單身起扣標準 90,501 元(財政部公告)。
 * 扶養每位免稅額 88,500 不動(等財政部後續公告;現況無人觸發、屬另案)。
 * rate 維持 6%(近似法,完整查表法另案)。
 */
export const TW_2026_WITHHOLDING_DEFAULTS = Object.freeze({
...
/**
 * 依薪資年度選用對應常數。規則:
 *   - 找 year ≤ 傳入年度 中年度最大的那筆(後向相容、年度切換 = 一次性 commit、不需改 caller)
 *   - 傳入年度 < 全部 → 回最早一筆(2024)
 *   - null/undefined → fallback 最新(避 caller 漏帶年度時用最舊年度)
 *
 * 邏輯範例:2024→2024、2025→2025、2026→2026、2027(未定義)→沿用 2026、2023→2024(回最早)
 */
const WITHHOLDING_DEFAULTS_BY_YEAR = Object.freeze([
  { year: 2024, defaults: TW_2024_WITHHOLDING_DEFAULTS },
  { year: 2025, defaults: TW_2025_WITHHOLDING_DEFAULTS },
  { year: 2026, defaults: TW_2026_WITHHOLDING_DEFAULTS },
]);

export function getWithholdingDefaults(year) {
  const list = WITHHOLDING_DEFAULTS_BY_YEAR;
  if (year == null || !Number.isFinite(year)) {
    return list[list.length - 1].defaults;
  }
  // 找 entry.year ≤ year 中、entry.year 最大的那筆
  let pick = null;
  for (const entry of list) {
    if (entry.year <= year && (!pick || entry.year > pick.year)) pick = entry;
  }
  // year 小於所有定義年度 → 回最早一筆(2024)
  if (!pick) return list[0].defaults;
