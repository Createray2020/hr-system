...
import { deductAnnualLeave, refundAnnualLeave, HOURS_PER_DAY } from './balance.js';
...
export async function submitLeaveRequest(repo, {
  ...
  // 餘額預檢(只 annual;comp 走 no-op,Batch 6 補)
  if (requiresBalance(lt) && getBalancePool(lt) === 'annual') {
  ...
  // TODO(Batch 6):leave_type='comp' 時做 comp_time_balance 預檢
...
export async function approveLeaveRequest(repo, { request_id, approved_by }) {
  ...
  if (pool === 'annual') {
    ...
    // TODO(Batch 6):呼叫 deductCompTime(repo, { employee_id, hours: finalizedHours, ... })
    // 本批 no-op,不檢查餘額也不扣減,讓申請流程能跑通。
  }
