...
export function findOverlayForRow(row, leaves, leaveTypeNameMap = {}) {
  ...
  const is_full_day = hours >= FULL_DAY_HOURS;
  const leave_name = leaveTypeNameMap[matched.leave_type] || matched.leave_type;
  return {
    ...
    hours,
    is_full_day,
    // Phase 1.3:UI 顯示用 short label、frontend 直接 render
    overlay_label_short: is_full_day ? '全日請假' : `半日請假 (${hours}h)`,
  };
}
 ...
 *
 * 階段 B1 Task 3:對 attendance row 同步偵測 post_hoc_leave (員工已打卡 + 事後核准請假)。
 * 規則:row.clock_in 不為 null AND 該日有 covering leave → post_hoc_leave=true
 * (UI 顯示「已打卡 + 補請假」⚠️ 警告、HR 手動處理 conflict、不自動扣 deduct)
 *
...
export function applyLeaveOverlay(rows, leaves, leaveTypeNameMap = {}) {
  return (rows || []).map(row => {
    const overlay = findOverlayForRow(row, leaves, leaveTypeNameMap);
    // post-hoc detection 只對 attendance row (有 clock_in 欄位) 有意義;
    // schedule row 沒 clock_in 欄位、isPostHoc 永遠 false
    const isPostHoc = !!(overlay && row?.clock_in);
    return { ...row, leave_overlay: overlay, post_hoc_leave: isPostHoc };
  });
}

/**
 * 階段 B1 Task 3:給 schedule rows 用,從 attendance rows 反推 post_hoc_leave。
 * (schedule row 自己沒 clock_in 欄位、得靠 attendance 補資訊)
 *
 * 規則:schedule row 同 (employee_id, work_date) 在 attendance 找到 clock_in 不為 null 的 row
 *       AND schedule.leave_overlay 不為 null → post_hoc_leave=true
 *
 * @param {Array} schedules - 已套 applyLeaveOverlay 的 schedule rows
 * @param {Array} attendances - 同期間的 attendance rows
 * @returns {Array}
 */
export function markPostHocFromAttendance(schedules, attendances) {
  const attClockedSet = new Set();
  for (const att of (attendances || [])) {
    if (!att?.employee_id || !att?.work_date || !att?.clock_in) continue;
    attClockedSet.add(`${att.employee_id}_${att.work_date}`);
  }
  return (schedules || []).map(s => {
    if (!s?.leave_overlay) return s;
    const key = `${s.employee_id}_${s.work_date}`;
    return { ...s, post_hoc_leave: attClockedSet.has(key) };
  });
}
