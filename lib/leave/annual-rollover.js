...
//      settlement_amount placeholder = 0、由月結時 lib/salary/settlement.js calculateSettlementAmount() reconcile
    ...
    if (old) {
      ...
      if (remainingDays > 0) {
        await repo.insertBalanceLog({
          ...
          changed_by: emp.id,
          reason: `annual rollover ${today}: settle remaining ${remainingDays} days (settlement reconciled at month-end)`,
        });
      }
    }
