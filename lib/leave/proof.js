...
//
// 對應 schema:
//   leave_types.requires_proof / proof_grace_days / proof_expiry_action(Phase 1.1 + 1.5)
//   leave_requests.proof_status / proof_due_at                          (Phase 1.1)
//
// 對應流程:
//   submitLeave 時:用 getInitialProofStatus 決定 proof_status 起始值、
//                  computeProofDueAt 算 proof_due_at(若 requires_proof)
//   cron 每日掃描:用 isProofExpired 判 row 是否過期、
//                  再依 leaveType.proof_expiry_action 分流 convert / mark_expired
//                  (分流邏輯在 lib/leave/proof-sweep.js)
...
/**
 * 判斷 leaveRequest 的證明是否已過期。
 * 純判定、不決定動作(動作分流看 leave_types.proof_expiry_action,在 proof-sweep.js)。
 *
 * 回 true 條件:proof_status='required' AND proof_due_at < now。
 * 其他都 false(包含 status='submitted' / 'expired' / 'converted_to_personal' /
 *               'not_required' / 沒 due_at)。
 *
 * @param {{ proof_status: string, proof_due_at: Date|string|null }} request
 * @param {Date|string} [now=new Date()]
 */
export function isProofExpired(request, now = new Date()) {
