...
//
// 規則:
//   - schedule 為 null:不算 late / early / early_arrival(都 0)、status 維持原值
...
//       early_leave > 0 → 'early_leave'
//       都 0 → 'normal'
//   - early_arrival_minutes 純 audit、不影響 status

import { calculateLateMinutes, calculateEarlyLeaveMinutes, calculateEarlyArrivalMinutes } from './clock.js';
 ...
 * @returns {{ late_minutes: number, early_arrival_minutes: number, early_leave_minutes: number, status: string }}
 */
export function recomputeAttendanceStatus(row, schedule) {
  const { clock_in, clock_out, work_date, status: existingStatus } = row;

  let lateMinutes = 0, earlyLeaveMinutes = 0, earlyArrivalMinutes = 0;
  if (schedule && clock_in) {
    lateMinutes         = calculateLateMinutes(clock_in, work_date, schedule.start_time);
    earlyArrivalMinutes = calculateEarlyArrivalMinutes(clock_in, work_date, schedule.start_time);
  }
