 ...
 *   pre_approval  : overtime_date >= today(事前、不可填過去;允許未來含跨月)
 *   post_approval : overtime_date === today(事後補僅限當日,超過當日不算加班)
 *
 * 純函式,呼叫端決定 today(API 用 Asia/Taipei、UI 用 client local)。
 *
 * @param {'pre_approval'|'post_approval'} request_kind
 * @param {string} overtimeDate  'YYYY-MM-DD'
 * @param {string} today         'YYYY-MM-DD'
 * @returns {{ ok: true } | { ok: false, reason: string }}
 */
export function checkOvertimeDateWindow(request_kind, overtimeDate, today) {
  if (!overtimeDate || !today) return { ok: false, reason: 'MISSING_DATE' };
  const d = String(overtimeDate).slice(0, 10);
  const t = String(today).slice(0, 10);
  if (request_kind === 'post_approval') {
    return d === t ? { ok: true } : { ok: false, reason: 'POST_APPROVAL_SAME_DAY_ONLY' };
  }
  return d < t ? { ok: false, reason: 'PRE_APPROVAL_NO_PAST' } : { ok: true };
}
