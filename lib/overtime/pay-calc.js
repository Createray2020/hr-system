...
/**
 * 加班 / 假日出勤的「平日每小時工資額」canonical 計算(勞基法 §2-4)。
 *
 * 基數應含經常性給付:base_salary + 全勤 + 職等 + 主管 + 額外加給;
 * 各 schema comment 已明寫 attendance_bonus / grade_allowance / manager_allowance
 * 「屬經常性給付、列入投保薪資」(migrations/2026_05_10_payroll_baseline.sql:22-24)。
 * extra_allowance 個案性(目前 prod 唯一 case = Ray「高階經理人加給」、固定每月),視為經常性。
 * allowance 欄全 prod 0 筆非零、未使用、仍納入公式以便未來啟用時自動生效。
 *
 * part_time:hourly_rate 已是 HR 設定的「含經常性全價」,不再疊加 allowance。
 *           修補 base_salary=0 → estimated_pay=0 的 bug(原 calcHourlyRate 只看 base_salary)。
 *
 * @param {{
 ...
 *   grade_allowance?: number|null,
 *   manager_allowance?: number|null,
 *   allowance?: number|null,
 *   extra_allowance?: number|null,
 * }} profile  employees 表常規欄位
 * @param {number} [monthlyWorkHoursBase]  預設 240、傳 null/undefined → 240、<=0 → 回 0
 * @returns {number}  時薪、round2、無效輸入回 0
 */
export function getOvertimeHourlyBase(profile, monthlyWorkHoursBase) {
  const base = monthlyWorkHoursBase == null ? 240 : Number(monthlyWorkHoursBase);
  if (!Number.isFinite(base) || base <= 0) return 0;
  if (!profile) return 0;

  if (profile.employment_type === 'part_time') {
    const h = Number(profile.hourly_rate) || 0;
    return h > 0 ? round2(h) : 0;
  }

  const monthly =
      (Number(profile.base_salary)       || 0)
    + (Number(profile.attendance_bonus)  || 0)
    + (Number(profile.grade_allowance)   || 0)
    + (Number(profile.manager_allowance) || 0)
    + (Number(profile.allowance)         || 0)
    + (Number(profile.extra_allowance)   || 0);
  return monthly > 0 ? round2(monthly / base) : 0;
}
