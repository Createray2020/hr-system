...
/**
 * Repo 介面契約：
 *   findApprovedPeriodsToLock(today): Promise<Array<{ id, employee_id, period_start, status, ... }>>
 *     找 status IN ('published','approved') 且 period_start <= today 的週期
 *     (函數名沿用歷史命名,實際接受 published 跟 approved 兩個來源狀態)
...
export async function runLockSweep(repo, today) {
  ...
  for (const p of (periods || [])) {
    ...
    await repo.logChange({
      ...
      before_data: { status: p.status, period_id: p.id },
