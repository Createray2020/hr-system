...
export function canManagerEditSchedule(period, manager, today, workDate) {
  ...
  // C5：published 之後主管只能改未來、HR/executive 不受限
  // 'published' 為 v2.5 規格用詞、目前 schema 仍是 approved/locked、三者都涵蓋
  const isPublished =
    period.status === 'published' ||
    period.status === 'approved' ||
    period.status === 'locked';
  // 2026-06 fail-closed:workDate 缺漏不再被當「無限制」放行;非 HR + isPublished 必須有
  // workDate 才能進 future-only 判斷。workDate 缺或 <= today 一律擋。
  if (!isHR && isPublished) {
    if (!workDate || String(workDate) <= String(today)) {
